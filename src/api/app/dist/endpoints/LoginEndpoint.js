"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.LoginEndpoint = void 0;
const Endpoint_1 = require("./Endpoint");
const jsonwebtoken_1 = __importDefault(require("jsonwebtoken"));
const HttpUtils_1 = require("../utils/HttpUtils");
const fs_1 = require("fs");
const dotenv_1 = __importDefault(require("dotenv"));
const Logger_1 = __importDefault(require("../utils/Logger"));
dotenv_1.default.config();
class LoginEndpoint extends Endpoint_1.Endpoint {
    get(request) {
        return __awaiter(this, void 0, void 0, function* () {
            Logger_1.default.info("Serving login page");
            const html = yield fs_1.promises.readFile("../../views/login.html", {
                encoding: "utf-8",
            });
            return new HttpUtils_1.Response(200, html);
        });
    }
    post(request) {
        return __awaiter(this, void 0, void 0, function* () {
            const { username, password } = request.body;
            const user = { username, password };
            const response = yield fetch("http://localhost:4000/login", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                },
                body: JSON.stringify(user),
            });
            if (response.status === 200) {
                const userData = yield response.json();
                Logger_1.default.info(`User ${username} logged in`);
                const token = jsonwebtoken_1.default.sign(userData, process.env.ACCESS_TOKEN_SECRET);
                return new HttpUtils_1.Response(200, JSON.stringify({ token }));
            }
            return new HttpUtils_1.Response(response.status, yield response.text());
        });
    }
}
exports.LoginEndpoint = LoginEndpoint;
